...
def main():
    ...
    det_model = init_detector(args.det_config, args.det_ckpt, f'cuda:{args.gpu_num}')
    assert det_model.CLASSES[0] == 'person', 'A detector trained on COCO is required'
    pose_model = init_pose_model(args.pose_config, args.pose_ckpt, f'cuda:{args.gpu_num}')

    results = []
    for anno in tqdm(my_part):
        # print(' ==> File name:', anno['filename'])
        frames = extract_frame(anno['filename'])
        if args.max_len != -1 and len(frames) > args.max_len:
            print(f'len(frames) {len(frames)} > max_len {args.max_len}', anno['filename'])
            frames = frames[:args.max_len]
